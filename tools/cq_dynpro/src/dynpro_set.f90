...
CONTAINS
  ...
  SUBROUTINE dynpro_read_input()
    ...
    NAMELIST /inputdynpro/ prefix, outdir, fileout, ndr, time_step, ct_atom,   &
         lcenter, lau_unit, lgrace, lsample, lrdf, lsphere, lsub, lvacf, lpsd, lnmr, &
         prefix_cqdata, lcq_coordforce, lspe

    NAMELIST /inputsample/ sp_start, sp_end, sp_n, iprint 
    
    NAMELIST /inputrdf/ rd_atom, rmin, rmax, nr
    
    NAMELIST /inputsphere/ sph_atom, sph_atn, sph_rcut, sph_n, sph_m, &
         sph_q, langle_spef, langle, angle_atom1, angle_atom2, ldist, ldist_spef, &
         dist_atom, sph_p
    ...
    NAMELIST /inputpsd/ m_cof, win, n_seg, loverlap, temp, qcfactor
